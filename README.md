...
此项目主要介绍在编程中常用的 21 种设计模式，根据包名区分不同的设计模式，首先要提醒的是，设计模式只是在语义上有所区分，但在语法上，很多设计模式区别不大，比如都要提取父类接口让子类灵活实现，会有似曾相识的地方；换句话说，不要纠结于设计模式的名字，遇到具体问题的时候，应该自然而然的，遵循正常人的思维，该怎么设计就怎么设计，至于用的是什么设计模式？无所谓的。

此项目中大部分的设计模式都有对应的示例代码，放于该设计模式的 Demo 包下面，每个 Demo 都很简单，适合于初学者。**一定记住，先学脉络，再学细节**，这是学习规律，看别人代码很容易陷入细节中，导致思维混乱，其实看不懂没关系，卡住了就先跳过，先学会怎么用，再去学为什么，学开车总不是先学汽车原理吧。

有人说，设计模式就是把简单的问题复杂化，没错，确实如此，但是它复杂得有理有据，就看你将来想在哪个方向上进行扩展，那它就在那个方向上产生多态、就在那个方向上产生抽象、就在那个方向上复杂化。站在使用者的角度开发，**没有思路的时候记住了，一旦需要灵活制定某些实现的时候，或者需要在将来可能会以不确定的方式来扩展系统的时候，抽象和多态是永远少不了的**。**可扩展性（Extensibility）是面向对象的核心，也是其精华所在，不然你感受不到系统之间精妙的结构那种乐趣**。

另外一点，在设计上面，**既不能不敢设计，也不能过度设计，更没有对错之分，最重要的是不停地动手写**。让实际应用和问题驱动学习，多思考，不会的就去查，每个阶段做出一点成果，会更容易坚持下去。学习是一个螺旋上升的过程，在上升过程中，成就感很重要，直接决定了一个人能不能继续学习下去。在工作层面上来讲，首要任务应该是先把功能实现，重构是在有余力或者有需要的情况下进行的。

**不要追求完美**。

（补充：很多 Demo 都用继承来写，但继承是一种耦合度很高的方式，要谨慎使用，能用接口最好用接口实现。那怎么决定该用接口还是抽象类呢？答案是都可以，是一回事，**不知道选哪个，那就用接口 interface，因为 implements 更灵活**。那如果非要区分开呢？可以这么划分，如果一个抽象概念在脑子里确确实实存在，比如动物、水果、书之类的，可以用抽象类，如果仅仅是一类事物在某个方面的特性，比如会飞的、能跑的、会叫的，用接口。）
...
下面介绍简单工厂，虽然它不属于正式的设计模式，但它经常作为基础形态被广泛讨论，特点为：一个工厂类，根据传入参数创建不同类型的对象，工厂类中包含了必要的逻辑判断，对于客户端来说，消除了与具体产品的依赖。实际应用如 Spring 的 bean 工厂，BeanFactory 根据传入的唯一标识（如 bean 名称）决定创建哪个 bean 对象，而无需关心对象的具体创建过程‌。

缺点是在新增产品时需要**修改原有的类，违反了开放-封闭原则。**比如在下文的 Demo 中，如果以后计算器程序要增加计算平方的功能，那么就需要修改原先的简单工厂类，为其增加 case 的分支条件。
...
有的地方也称其为普通工厂，名字无所谓，知道就行。特点为：定义了一个工厂类的接口，让工厂接口的子类决定具体实例化哪个类，延迟对象的创建。新增产品时无需修改旧代码，符合开放-封闭原则。举例说明：  
a、Calendar.getInstance() 方法：根据默认时区或指定参数返回具体的日历实例。   
b、DriverManager.getConnection() 方法：根据传入的参数返回具体的数据库连接对象。

缺点是每加一个新产品，就需要加一个工厂子类，增加了额外的开发量。而且，工厂方法模式把简单工厂的内部逻辑判断转移到了客户端来进行，客户端需要决定实例化哪一个子类工厂，**选择判断的问题依然存在。**
...
提供接口创建一组相关或依赖的对象家族，强调产品间的组合关系而非单一对象的创建。此模式在 JDK 和许多开源框架（比如 Spring Boot）中随处可见，它们很容易被发现：任何用于创建对象但返回接口或抽象类的，大概率就是抽象工厂。
...
a、一个系统访问数据库表的客户端不可能只有一个，因此如果整个系统要更换使用的数据库的话，那么就要将所有客户端中的：（例如）    IFactory factory = new OracleFactory();  
改为：    IFactory factory = new SqlServerFactory();  
这**不能实现改动一处就更改全部**的要求。

b、如果以后客户端要访问一张新表，例如要新增部门表（Department），那么除了要增加 「IDepartmentOperation、OracleDepartOper、SqlServerDepartOper」 这三个新的类以外，还要修改原有的 「IFactory、OracleFactory、SqlServerFactory」 这三个工厂类才可以完全实现，这样**大批量地改动原有类**的做法是不好的。
...
优点：  
反射是**用字符串来实例化对象**，而字符串是可以拼接和更换的，因此可以将变化放到配置文件中，后续要切换的时候只用改配置文件，代码一行不用动，从而消除 switch 或 if 判断语句带来的耦合。所有用到简单工厂的地方，都可以考虑用反射代替。

Demo3：  
用反射 + 配置文件的方式改进 Demo2（其实改进的类只有 DataAccess 类）。

**小结**：  
工厂方法易于扩展新的产品品种，但不适合创建系列产品，会导致工厂类泛滥；而抽象工厂恰好相反，它便于扩展系列产品，但不适合增加新的产品品种，会导致改动太多。结合了两者优点的模式是不存在的，在实际应用时，选用标准为**降低耦合度的目的是否达到**。对于抽象工厂，建议使用配置文件加反射的方式实现。

一句话区分：  
简单工厂：一个工厂类包揽所有产品，由参数决定造什么（新增产品要改代码）；  
工厂方法：每个产品配专属工厂（苹果厂造苹果，华为厂造华为），新增产品无需改旧代码；  
‌抽象工厂：一个工厂直接造整套配套产品（如华为厂造手机 + 耳机 + 电脑），解决‌产品家族的问题。
...
适用场景：  
动态地给一个对象添加一些额外的职责，就增加功能来说，装饰模式比生成子类更灵活，应用较广。

当系统需要新功能的时候，我们有可能向旧类中加入新的字段、方法或者逻辑，这样的做法增加了旧类的复杂性，而往往这些新加入的东西仅仅是为了满足一些只在某种特定情况下才会执行的需求。装饰模式就对这种情况提供了一个很好的解决方案，它把每个要新增的功能放在单独的类中，并让这个类包装它所要装饰的对象，装饰类与被装饰类实现相同的接口，调用方式一致，这样客户代码就可以根据需要有选择地、按顺序地进行多重装饰。

缺点：  
如果装饰类与被装饰类的关注点完全不同，那么就不应该让它们实现同一接口。除此之外，如果未实现接口的业务类也想被装饰该怎么办呢？
...
说明：  
如果不使用建造者模式，那么先画一个瘦的小人，再画一个胖的小人，在画第二个胖的小人的时候就可能因为疏忽而忘记画某一个部位，导致建造出来的对象不对。当建造的步骤逐渐增多时，这种出错遗漏的概率就越大。
...
适用场景：  
希望复用一些现存的类，但是接口与复用环境要求不一致的情况。比如需要复用一些早期代码的功能，因此适配器模式通常是在双方接口都不太容易修改时，在软件开发后期或维护期才考虑使用。模式虽好，但不能滥用哦。简单来说，凡是输入接口和输出接口不一样的，就可以认为是一个适配器，这跟现实中的电源适配器是一个意思，输入电压 220V，输出电压 11V。
...
Demo：  
记录游戏角色状态，保存游戏进度。Demo1 是不好的做法，写出来为了提醒自己不犯同样的错误，推荐 Demo2 的做法。

&nbsp;
<img src="UMLDiagrams/memento.png" width="75%" />

优点：  
使用备忘录可以把复杂对象的内部信息对其他的对象屏蔽起来。当角色状态改变的时候，有可能这个状态无效，这时候就可以使用暂存的备忘录将状态复原。

缺点：  
以 Demo2 为例，角色状态需要完整存储到备忘录对象中，如果状态数据很大很多，那么在资源的消耗上，备忘录对象会非常耗内存。
...
适用场景：  
当不变的和可变的行为在方法的子类实现中混合在一起时。实际应用如 RestTemplate、JpaTemplate、JmsTemplate 等。

优点：  
模板方法模式将不变的行为搬移到了超类，通过在超类中定义公用的算法结构，去除了子类中的重复代码。子类可以根据具体需求覆盖超类中的抽象方法，从而实现不同的具体行为。

缺点：  
父类定义了所有要执行的非功能性代码以及它们的执行顺序，子类只能无条件接受，即使某个子类根本不需要某个非功能性需求（比如事务），那它也没有办法把该部分的代码去掉。另外，父类与子类紧密相关，如果父类的结构被改动，则可能也需要同步修改所有子类的实现逻辑。
...
适用场景：  
一般在初始化信息不发生变化的情况下，用克隆是最好的办法。这样不仅隐藏了对象创建的细节，而且由于不用重新初始化对象，而是动态地获得对象运行时的状态，性能也大大提高。在我们的业务代码中，经常要使用到的各种 DTO、BO、DO、VO 间的转换，就可以参考原型模式的思想来做。

Demo：  
简历复印，并且有可能修改其中的某份简历。Demo1 为浅复制，Demo2 为深复制。
...
（B）**CGLib 动态代理**（默认采用，Code Generation Library）：基于继承实现，只要目标类没有被标记为 final 就可以，代理类继承目标类，它使用 ASM 框架生成目标类的子类（代理类）字节码，通过在子类中重写父类（目标类）的方法实现对方法的增强，代理类与目标类是父子关系（与 JDK 动态代理类似，CGLib 创建代理实例‌的逻辑封装在 Enhancer 类中，具体的代理业务通过实现 MethodInterceptor 接口来定义）。

代理模式的适用场景：

- 远程代理：也就是为一个对象在不同的地址空间提供局部代表，这样可以隐藏一个对象存在于不同地址空间的事实。例如：在创建 WebService 客户端的时候，我们会引用一个 WebService 地址，然后生成一些相关的客户端文件，其实这些文件就是代理，使得我们可以通过调用它们就能实现远程访问，而在使用时就跟调用本地方法一样简单。
- 虚拟代理：根据需要创建开销很大的对象，然后通过它来存放实例化需要很长时间的真实对象，这么做可以提高性能。例如：当我们打开一个很大的 HTML 网页时，文字能很快看到，但是图片需要一张一张下载后才能看到。那些未打开的图片框，就是通过虚拟代理替代了真实的图片，此时代理存储了真实图片的路径和尺寸。
- 安全代理：用来控制访问真实对象时的权限，起保护作用。
- 智能指引：是指当调用真实的对象时，代理去处理另外一些事。比如当对象没有引用时，代理去自动释放它；或是在访问一个实际对象前，代理去检查它是否已经被锁定等等。它们都可以通过代理去处理一些附加的操作。
...
适用场景：  
对象间多对多的复杂交互。此模式将网状结构转换成星型拓扑结构，很好地应用了迪米特法则。实际应用包括我们熟知的 Java Swing 的事件机制，它就是以 JFrame 窗体作为中介来管理各个具体控件间的交互，除此之外还有 MQ 也是。

Demo：  
由联合国安理会处理美国、伊拉克等国家之间的问题。

&nbsp;
<img src="UMLDiagrams/mediator.png" width="70%" />

缺点：  
由于控制集中化，中介者类的职责将越来越多，交互的复杂性变成了中介者的复杂性。因此集中控制既是中介者模式的优点，也是它的缺点，使用时要考虑清楚。
...
适用场景：  
需要在不同时间或场景下应用不同的业务规则。这些规则可以理解为一个个算法，策略模式就是用来封装算法的，而且算法之间可以互相替换，且替换时不会影响到使用算法的客户端。

在 JDK 中，比较器 Comparator 接口就用到了这个模式（常与 Comparable 接口配合使用，首先让类实现 Comparable 接口来表明自己是可比较的（这样能让排序方法通用），但具体根据什么方式/什么策略来比较大小，则引用 Comparator 接口，让它的子类来实现）；此模式也可用于优化大量的 if...else 语句。

Demo：  
开发一款商场的收银软件，包括正常情况下的收银和有促销活动下的收银。
...
适用场景：  
当一个对象的行为取决于它的状态，并且它可能在运行时根据状态改变它的行为时。这个模式非常常用，因为一个系统中不可能没有状态，比如常见的订单状态、付款状态、物流状态、任务状态、网络状态等等，都能用上。

Demo：  
展现一个员工在不同时间段工作状态的变化。比如：上午精神好，中午想睡觉，下午渐恢复，加班苦煎熬。
...
Demo：  
烧烤店点烧烤。（如果是烧烤摊，则接受请求、执行操作等都由一人完成，耦合太紧，客人太多时就容易出错）
...
Demo：  
不同的商家都要购买网站，但网站的功能是类似的。使用享元模式后就不用再给每一个客户都单独开发一个网站了，共享一个网站的代码即可。
...
适用场景：  
当一个对象的状态改变需要同时改变其他对象，而且它不知道具体有多少对象有待改变时（或者说所有依赖于它的对象都会得到通知被动更新），就可以用这个模式。实际应用如 Spring 中的监听器 ApplicationListener，还有一般用到 ZooKeeper 的地方，都会使用这个模式，比如分布式锁、服务发现等。

好处：  
解耦，让耦合的双方都依赖于抽象，而不是依赖于具体。从而使得各自的变化都不会影响另一边的变化。 
...
Demo3：  
在这先举一个应用观察者模式的例子。当我们在开发工具 eclipse 中写完一段程序，用 Debug as 运行时，eclipse 的界面会发生变化：控制台窗体弹出，工具栏也出现很多关于调试的按钮......这一个小小的点击动作就引发了这么多的变化，涉及到很多不同的控件。

问题在于这些控件可能不是出自同一制造商，而且它们都是被封装好的，不同制造商的方法名是绝对不同的，因此也就不可能让每个控件都去实现同一个观察者接口，抽象观察者失去作用。为了解决这一问题，Demo3 采用了**事件委托机制**来实现。eclipse 也是插件架构最经典的应用案例。

委托是一种引用方法的类型。一旦为委托分配了方法，委托将与该方法具有完全相同的行为。委托方法的使用可以像其他任何方法一样，具有参数和返回值。委托可以看作是对函数的抽象，是函数的「类」，委托的实例将代表一个具体的函数。

一个委托可以搭载多个方法，使用时所有方法将被依次唤起。这些搭载的方法必须具有相同的参数列表和返回值类型，但不需要属于同一个类。
...
适用场景：  
1.请求数据在进入具体业务之前，对请求做一系列前置处理。  
这一系列的前置处理被封装为一个个处理器类，并且可以灵活增减处理器，连接这些处理器后就像链子一样，每个请求按照处理链的顺序依次进行处理，直到请求传递完或者请求被拒绝。比如在文本数据入库前，需要先删掉文本中的特殊符号、替换掉敏感词、自动填充记录的操作人 ID 和操作时间；或者在服务端处理用户请求之前，对请求数据先做一些过滤、校验的工作；又或者在将响应返回给客户端之前，对响应数据先做一些包装、修饰的工作等等，都可以用自定义的拦截器来完成。  
在 Java 框架中，**凡是带有 Filter 关键词**，**或者用到拦截器的地方**基本都是用的这个模式，并且往往是在同一个过滤器中既处理请求 Request，又处理响应 Response，只是数据被过滤器链条处理的顺序正好相反，比如：

- Java Web 中的 Servlet：  
  Servlet 是服务器端程序，是衔接前后端的一种技术，也是 JavaEE 规范的核心组件，其主要功能是通过 HTTP 接收和响应来自 Web 客户端的请求，主要运行在 Web 服务器中（如 Tomcat、Jetty 等）。以 Tomcat 为例，其实 Servlet 也是 Tomcat 的一个组件，或者说 Tomcat 是一个 Servlet 的容器，因为 Servlet 的功能依赖于 Tomcat 提供的 servlet-api.jar，Tomcat 运行时，会读取 web.xml 文件，然后初始化 ServletConfig、ServletContext 等对象，此时，Servlet 才具备相关的功能。  
  狭义的 Servlet 是指一个接口，广义的 Servlet 是指任何实现了这个 Servlet 接口的类，一般理解为后者。传统的配置方式是在 XML 文件中定义 Servlet 和 URL 的映射关系，后来通过 Spring Boot 等框架的封装简化了它的使用，使用注解即可自动配置（如 @GetMapping）。
- Struts2 中的 Interceptor。
...
适用场景：  
此模式的目的是要把处理从数据结构中分离出来。因此当数据结构相对稳定，而算法又易于变化时，用此模式较好。所以它的优点是增加新的操作很容易，缺点是增加新的数据结构很困难。

GOF 中有人说过，「大多时候你并不需要访问者模式，但是一旦你需要访问者模式的时候，那就是真的需要它了」。
...
概念：  
如果两个类不必彼此直接通信，那么这两个类就不应当发生直接的相互作用。当其中一个类需要调用另一个类的某个方法时，可通过第三者转发这个调用。

通俗解释：  
新员工刚入职时，想认识老员工不需要自己一个一个地去问，找人事部的人作为中间者介绍即可；电脑或技术问题，也只需要认识技术部负责人后再找对应的人即可。对于新员工来说，有直接作用的只有负责人，且目的都能达到，还比自己挨个去问的效率更高。
...
好处：  
此原则跟里氏代换原则是分不开的，针对接口编程以后，子类就可以相互替换，而且不会影响到使用父类类型的模块。
