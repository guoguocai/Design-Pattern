...
首先介绍简单工厂，虽然它不属于正式设计模式，但它经常作为基础形态被广泛讨论，特点为：一个工厂类，根据传入参数创建不同类型的对象，工厂类中包含了必要的逻辑判断，对于客户端来说，去除了与具体产品的依赖。
...
**小结**：  
工厂方法易于扩展新的产品品种，但不适合创建系列产品，会导致工厂类泛滥；而抽象工厂恰好相反，它便于扩展系列产品，但不适合增加新的产品品种，会导致改动太多。结合了两者优点的模式是不存在的，在实际应用时，选用标准为**降低耦合度的目的是否达到**。对于抽象工厂，建议使用配置文件加反射的方式实现，比如 Spring 的 bean 工厂就是这么做的。
...
概念：  
为对象提供一种代理用来控制对此对象的访问，可理解为代理就是真实对象的代表，应用广泛。分为静态代理（编译期，代理类是程序员写好的）和动态代理（运行期，代理类是动态生成的，动态生成的逻辑自然也是由程序员来写，只不过一般写好后就不需要再去改动）。原先的类不用进行任何改变，直接访问代理类即可。
