...
（补充：很多 Demo 都用继承来写，但继承其实是一种耦合度很高的方式，要谨慎使用，能用接口最好用接口实现。那怎么决定该用接口还是抽象类呢？答案是都可以，是一回事，**不知道选哪个，那就用接口 interface，因为 implements 更灵活**。那如果非要区分开呢？可以这么划分，如果一个抽象概念在脑子里确确实实存在，比如动物、水果、书之类的，可以用抽象类，如果仅仅是一类事物在某个方面的特性，比如会飞的、能跑的、会叫的，用接口。）
